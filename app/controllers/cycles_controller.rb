...
class CyclesController < ApplicationController
  ...
  def index
    @cycles = Cycle.find_by_user current_user
  end
  
  def edit
    @cycle = Cycle.find(params[:id], :include => :day)
    authorize
  end
  ...
  def update
    ...
    authorize
    @cycle.update_attributes(params[:cycle])
    redirect_to cycles_url
  end
  
  def show
   @cycle = Cycle.find_by_user_and_id(current_user, params[:id].to_i)
  ...
  def authorize
    access_denied unless current_user.id == @cycle.day.user_id
  end
end
